...
문제를 해결하려면 아래의 내용을 고려해야한다.
- 겹쳐진 직사각형 영역의 테두리
- 너비 1칸짜리 사각형인 경우 도형을 건너가는 문제
- 테두리를 따라 최소 거리 탐색

각각 해결하기 위해 아래의 방식을 사용함
1. map[][] 배열에서 0은 아무것도 없는 칸, 1은 도형의 테두리, 2는 도형의 안
2. 좌표계에 Scale(2배)를 두어 dx/dy 전략에서 건너가는것을 방지함
3. DFS를 구현하여 최소거리 탐색
